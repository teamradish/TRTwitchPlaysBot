...
namespace TRBot.Data
{
    ...
    public static class LeaderboardHelper
    {
        ...
        public static (int, int) GetPositionOnCreditLeaderboard(string userName)
        {
            using (BotDBContext context = DatabaseManager.OpenContext())
            {
                ...
                int leaderBoardCount = context.Users.Include(u => u.Stats).Where(u => u.Stats.OptedOut <= 0).Count();
                ...
                int position = context.Users.Include(u => u.Stats).Where(u => u.Stats.Credits >= user.Stats.Credits && u.Stats.OptedOut <= 0).OrderByDescending(u => u.Stats.Credits).Count(u => u.Stats.Credits > user.Stats.Credits);
        ...
        /// </returns>
        public static SortedDictionary<int, (long, string)> GetCreditLeaderboardSubset(string userName, in int surroundingCount)
        {
            using (BotDBContext context = DatabaseManager.OpenContext())
            {
                ...
                List<User> allUsers = context.Users.Include(u => u.Stats).Where(u => u.Stats.OptedOut <= 0).OrderByDescending(u => u.Stats.Credits).ToList();
        ...
        /// <summary>
        /// Gets a user's position in relation to all others based on input count.
        /// </summary>
        /// <param name="userName">The name of the user.</param>
        /// <returns>An tuple of integers; the first represents the user's position on the input leaderboard,
        /// and the second represents the number of unique input counts on the leaderboard.
        /// If the user is not found, the returned user position is -1.</returns>
        public static (int, int) GetPositionOnInputLeaderboard(string userName)
        {
            using (BotDBContext context = DatabaseManager.OpenContext())
            {
                User user = DataHelper.GetUserNoOpen(userName, context);
                ...
                //Get the position - take all inputs greater than or equal to the user's inputs
                //Order them in descending order, and get the count of users greater than current count
                //Don't include opted out users
                int position = context.Users.Include(u => u.Stats).Where(u => u.Stats.ValidInputCount >= user.Stats.ValidInputCount && u.Stats.OptedOut <= 0).OrderByDescending(u => u.Stats.ValidInputCount).Count(u => u.Stats.ValidInputCount > user.Stats.ValidInputCount);

                //We have how many users have more inputs than this one, so add 1 to get the user's actual position
                position += 1;
        ...
        /// <summary>
        /// Gets a subset of the inputs leaderboard around a given username.
        /// </summary>
        /// <param name="userName">The name of the user.</param>
        /// <param name="surroundingCount">The total number of entries to obtain around the given username.
        /// This will be spread out as evenly as possible.
        ...
        /// <returns>A SortedDictionary with integer keys and tuple values.
        /// The key is the zero-based position in the leaderboard.
        /// The tuple's first value is a long representing the number of inputs the user has.
        /// The second value is a string containing the user's name.
        /// If the user is not found, the returned SortedDictionary is null.
        /// </returns>
        public static SortedDictionary<int, (long, string)> GetInputLeaderboardSubset(string userName, in int surroundingCount)
        {
            using (BotDBContext context = DatabaseManager.OpenContext())
            {
                User namedUser = DataHelper.GetUserNoOpen(userName, context);
                ...
                //Order them in descending order, and get the count of users greater than current count
                //Don't include opted out users, but always include the user we're looking for the subsets around
                List<User> allUsers = context.Users.Include(u => u.Stats).Where(u => u.Stats.OptedOut <= 0).OrderByDescending(u => u.Stats.ValidInputCount).ToList();
                ...
                //Add users with input values greater
                //Keep going until either the top of the list or we found half of the surrounding value
                for (int i = userIndex - 1, foundCount = 0; i >= 0 && foundCount < (surroundingCount / 2); i--, foundCount++)
                {
                    User user = allUsers[i];
                    leaderboardDict[i] = (user.Stats.ValidInputCount, user.Name);
                }
                
                //Add the user
                leaderboardDict[userIndex] = (namedUser.Stats.ValidInputCount, namedUser.Name);

                //Add users with input values lower
                //Keep going until either the end of the list or we found the rest of the surrounding value
                for (int i = userIndex + 1, foundCount = leaderboardDict.Count - 1; i < allUsers.Count && foundCount < surroundingCount; i++, foundCount++)
                {
                    User user = allUsers[i];
